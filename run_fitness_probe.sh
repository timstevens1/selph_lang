...
# Concatenate and track offsets for combined
OFFSET=0
echo "Combined curriculum file offsets:" >&2
for f in "${COMBINED_FILES[@]}"; do
    LINES=$(wc -l < "$f")
    echo "  $f: offset=$OFFSET, lines=$LINES" >&2
    OFFSET=$((OFFSET + LINES))
done
cat "${COMBINED_FILES[@]}" > "$COMBINED"
...
# Concatenate and track offsets for post-mortem
PM_OFFSET=0
echo "Post-mortem file offsets:" >&2
for f in "${PM_FILES[@]}"; do
    LINES=$(wc -l < "$f")
    echo "  $f: offset=$PM_OFFSET, lines=$LINES" >&2
    PM_OFFSET=$((PM_OFFSET + LINES))
done
cat "${PM_FILES[@]}" > "$PM_COMBINED"
echo '(define run-post-mortem (lambda (results) (run-fitness-post-mortem results)))' >> "$PM_COMBINED"

echo "Combined curriculum: $COMBINED ($OFFSET lines total)" >&2
echo "Post-mortem: $PM_COMBINED ($PM_OFFSET lines total)" >&2
